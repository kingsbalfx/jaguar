  ...
  const sendSignal = async (event) => {
    ...
    try {
      ...
      const mt5Text = data.mt5Execution?.attempted
        ...
        : "";
      const planText = data.audienceByPlan
        ? ` Plans: ${Object.entries(data.audienceByPlan).map(([plan, count]) => `${plan} ${count}`).join(", ") || "none"}.`
        : "";
      const smtpText = data.smtp
        ? ` SMTP: ${data.smtp.configured ? `${data.smtp.provider} ${data.smtp.sender || ""}` : "not configured"}.`
        : "";
      const errorText = data.errors?.length ? ` Errors: ${data.errors.slice(0, 2).map((item) => item.error).join(" | ")}.` : "";
      setSignalStatus(`Signal delivered. Audience ${data.audience}; ${data.emailed} email${data.emailed === 1 ? "" : "s"} sent; ${data.notified} dashboard alert${data.notified === 1 ? "" : "s"} created; ${data.skippedQuota} skipped by daily quota.${planText}${smtpText}${errorText}${mt5Text}`);
    ...
    <main className="container mx-auto p-4 sm:p-6">
      ...
      <section className="bg-white/5 rounded-lg p-4 mb-6">
        ...
        <div className="mt-4 rounded-md border border-white/10 bg-black/30 p-3">
          <div className="text-xs uppercase tracking-widest text-sky-200">Generated By Bot</div>
          <div className="mt-1 text-sm text-gray-300">
            Today: {generatedSignalStats?.generatedToday ?? 0} | Last 7 days: {generatedSignalStats?.generatedWeek ?? 0} | Total: {generatedSignalStats?.generatedTotal ?? 0}
          </div>
          <div className="mt-2 grid gap-3 md:grid-cols-3">
            <div>
              <div className="text-xs text-gray-400">Today pairs</div>
              {renderPairList(generatedSignalStats?.topGeneratedPairsToday || [])}
            </div>
            <div>
              <div className="text-xs text-gray-400">Week pairs</div>
              {renderPairList(generatedSignalStats?.topGeneratedPairsWeek || [])}
            </div>
            <div>
