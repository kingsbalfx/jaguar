 ...
 *  - active link highlighting using usePathname() (Next 13+)
 *  - accessible menu button, keyboard focus styles
 *
 * Tailwind classes are used — tweak colors to match your theme.
 */
...
export default function Header() {
  ...
  // close mobile menu on navigation or when path changes
  useEffect(() => setOpen(false), [pathname]);

  const isActive = (href) => {
    if (!href) return false;
    // simple active check: exact match or startsWith for nested routes
    if (href === "/") return pathname === "/";
    return pathname === href || pathname.startsWith(href + "/");
  };

  return (
    <header className="w-full bg-transparent py-4 px-4 md:px-6">
      ...
      <div
        id="mobile-menu"
        className={`md:hidden mt-2 bg-black/30 backdrop-blur-sm ${open ? "block" : "hidden"}`}
      >
